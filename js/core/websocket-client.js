...
export class MultimodalLiveClient extends EventEmitter {
    ...
    /**
     * Sets the OAuth access token for Google API tools.
     * This should be called after the user authenticates with Google.
     *
     * @param {string} token - The OAuth access token from Firebase Google Auth.
     */
    ...
    connect(config) {
        ...
        // Do not send an empty tools array. Keeping tools off by default reduces model
        // planning overhead and avoids unnecessary tool-call round trips in live voice mode.
        if (tools.length > 0) {
    ...
    async receive(blob) {
        ...
        if (response.serverContent) {
            ...
            // User mic transcription (what we said) — streams in deltas, finalized at turn end.
            if (serverContent.inputTranscription) {
                this.emit('inputtranscription', serverContent.inputTranscription);
            }
            // Model output transcription (captions for the spoken audio).
